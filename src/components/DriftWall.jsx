...
const DriftWall = ({
  ...
  const columnItems = useMemo(() => {
    ...
    // To ensure variety when columns > items/3, we fill each column with a random mix of items
    for (let c = 0; c < columns; c++) {
      const shuffled = [...items].sort(() => 0.5 - Math.random());
      // Give each column 4 distinct random items to cycle through
      cols[c] = shuffled.slice(0, Math.min(4, items.length));
    }
    return cols;
  }, [items, columns]);
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span className="drift-wall__inner flex flex-col items-center justify-center gap-3">
        <div className="bg-slate-100 rounded-2xl p-3 flex items-center justify-center shadow-inner">
          <img 
            src={item.image} 
            alt={item.title ?? ''} 
            loading="lazy" 
            decoding="async" 
            draggable={false}
            style={{ width: '64px', height: '64px', objectFit: 'contain', padding: 0, backgroundColor: 'transparent' }}
          />
        </div>
        {item.title && (
          <span className="text-sm font-mono font-bold text-white text-center z-10 px-1 drop-shadow-md">
            {item.title}
          </span>
        )}
