    ...
    <section id="contact" className="w-full py-12">
      ...
      <div className="glass-card p-8 md:p-12 w-full max-w-3xl mx-auto flex flex-col md:flex-row gap-12 items-center justify-between">
        <div className="flex flex-col gap-6 w-full">
          <a href={`mailto:${t.contact.email}`} className="flex items-center gap-4 text-muted-foreground hover:text-accent transition-colors group w-fit">
          ...
          <a href={`https://wa.me/${t.contact.phone.replace(/[^0-9+]/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-muted-foreground hover:text-accent transition-colors group w-fit">
            <div className="p-3 border border-card-border rounded-lg group-hover:border-accent bg-background">
              <Phone className="h-5 w-5" />
            </div>
            <div className="flex flex-col">
              <span className="text-xs font-mono uppercase">{language === 'es' ? 'Teléfono' : 'Phone'}</span>
              <span className="text-foreground font-medium">{t.contact.phone}</span>
            </div>
          </a>

          <a href={t.contact.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-muted-foreground hover:text-accent transition-colors group w-fit">
            <div className="p-3 border border-card-border rounded-lg group-hover:border-accent bg-background">
              <LinkIcon className="h-5 w-5" />
            </div>
            <div className="flex flex-col">
              <span className="text-xs font-mono uppercase">LinkedIn</span>
              <span className="text-foreground font-medium">{t.contact.linkedinLabel}</span>
            </div>
          </a>

          <a href={t.contact.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-muted-foreground hover:text-accent transition-colors group w-fit">
            <div className="p-3 border border-card-border rounded-lg group-hover:border-accent bg-background">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-foreground font-medium">{t.contact.githubLabel}</span>
            </div>
          </a>
        </div>
